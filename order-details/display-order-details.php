...
if(isset($_GET["id"])){
    $orderId = $_GET["id"];
    $selectOrder = "select productImage, productName, productPrice, quantity from productorder po, product p where po.orderId=".$orderId. " and po.productId = p.productId";  
    $users=new ORM();
    $connect=$users ->connect('cafateria','3306','root','sayed771995');
    $res=$users -> executeQuery($selectOrder);
    $records=$res -> fetchAll(PDO::FETCH_ASSOC);
    
    
    // select `productImage`, `productName`, `productPrice`, `quantity` from productorder po, product p where po.orderId=1 and po.productId = p.productId;
